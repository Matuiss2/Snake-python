...
# V 1.32 - Few performance improvements(direction, sounds) and redability
    ...
    while True:
        ...
        if not paused:
            ...
            if not food_spawned:  # When a food is taken it ll respawn randomly
                food_pos = (random.randrange(1, 72) * 10, random.randrange(1, 46) * 10)
            food_spawned = True    # Keep the food spawn cycle forever
            # Drawing
            ...
            pygame.draw.rect(player_screen, orange, pygame.Rect(food_pos[0], food_pos[1], 10, 10))
            if snake_pos[0] not in range(0, 711) or snake_pos[1] not in range(0, 451):
